...
**Opção A (recomendada para começar):** um contêiner rodando `nginx` + `fcgiwrap` + `git-http-backend`. São ~30 linhas de configuração. Autenticação por token via um endpoint de autorização que consulta sua API. Roda numa VPS de US$ 20.
 
**Opção B:** Gitea ou Forgejo (servidores git prontos, de código aberto) rodando escondidos atrás da sua aplicação. Você ganha permissões, usuários e propostas de mudança prontos, e usa a API deles a partir do seu site. Reduz meses de trabalho; o custo é depender do modelo de dados deles.
...
Recomendação: **B para o MVP, A quando a escala justificar**.
...
- Next.js na Vercel. O conteúdo é texto pequeno e majoritariamente público: cacheie tudo agressivamente na borda.
...
| Semanas | Entrega |
|---|---|
| 1–2 | Modelo de dados, autenticação, Gitea escondido, push e clone funcionando |
