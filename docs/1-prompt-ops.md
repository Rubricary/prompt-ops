...
> **Precedência.** `prd.md` e `sdd.md` são canônicos onde houver conflito com este documento. Divergências conhecidas: a tabela de monetização abaixo foi substituída pela do `1_2` (fases 0–4); o formato do repositório é `prompt.md` + `evals.yaml`, sem `config.yaml` separado (`sdd.md` §3); e o runner de evals citado como "ver Hub 2" é parte do próprio Prompt Ops, não um serviço externo.
