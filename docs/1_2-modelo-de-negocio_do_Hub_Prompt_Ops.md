...
> **Precedência.** `prd.md` e `sdd.md` são canônicos onde houver conflito. Já refletido abaixo: formato de dois arquivos e teto da Fase 0 em 50 prompts.
...
- **Formato canônico fechado.** `prompt.md` (com a configuração no frontmatter) + `evals.yaml`. Congelar o formato antes de produzir em escala — mudar o esquema com 200 itens prontos custa uma semana de retrabalho. Spec em `sdd.md` §3.
- **50 prompts em inglês**, cobrindo três setores por inteiro — Support 20, Sales 15, Engineering 15 —, cada um com bateria de testes e nota contra 3 modelos (ver `1_1-taxonomia-e-semeadura.md`). 150–300 é a meta do primeiro ano, não do lançamento: três setores completos parecem mais cheios que dez rasos.
...
Por isso o silêncio precisa de teto: **90 dias ou 50 prompts com nota, o que vier primeiro.** Passado o teto, lança-se com o que existe. "Ainda não está bom" é o argumento que mantém projeto em gaveta por dois anos.
